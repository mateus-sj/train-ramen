...
parser.add_argument('--tgt_model', default='', help='save the target model state dict')
...
if 'roberta' in params.src_model:
    ...
    MASK_TOKEN, MASK_INDEX = '<mask>', 52008
...
def guess(src_embs, src_bias, tgt_tokenizer, src_tokenizer, prob=None):
    ...
    for t, ws in prob.items():
        if not tgt_tokenizer.token_to_id(t):
          continue
        ...
        embs_final = px @ src_embs[ix]
        bias_final = px.dot(src_bias[ix])
        tgt_embs[ti] = embs_final
        tgt_bias[ti] = bias_final
    return tgt_embs, tgt_bias
...
def init_tgt(params):
    ...
    src_embs = model.state_dict()[MAP['word_embeddings']]
    src_bias = model.state_dict()[MAP['output_bias']]
    ...
    # model.state_dict()[MAP['word_embeddings']] = tgt_embs
    # model.state_dict()[MAP['output_bias']] = tgt_bias
    # model.state_dict()[MAP['output_weight']] = model.state_dict()[MAP['word_embeddings']]
    # Create a new state_dict with the updated embeddings and bias
    new_state_dict = model.state_dict()
    new_state_dict[MAP['word_embeddings']] = tgt_embs
    new_state_dict[MAP['output_bias']] = tgt_bias
    new_state_dict[MAP['output_weight']] = tgt_embs  # or whatever you intend here

    # Load the new state_dict into the model
    model.load_state_dict(new_state_dict)

    print(' checksum tgt | embeddings {:.5f} - bias {:.5f}'.format(
        model.state_dict()[MAP['word_embeddings']].norm().item(),
        model.state_dict()[MAP['output_bias']].norm().item()))
    # save the model
    torch.save(model.state_dict(), params.tgt_model)
